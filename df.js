...
(function () {
	...
	df.result = function(object, prop, fallback) {
		var value = object == null ? void 0 : object[prop];
		if(value === void 0) {
			value = fallback;
		}
		return df.isFunction(value) ? value.call(object) : value;
	}
	var idCounter = 0;
	df.uniqueId = function(prefix) {
		var id = ++idCounter + '';
	...
	var unEscapeMap = df.invert(escapeMap);
	var createEscaper = function(map) {
		var escaper = function(match) {
			return map[match];
		}

		var source = '(?:' + df.keys(map).join('|') | ')';
	...
	df.escape = createEscaper(escapeMap);
	df.unescape = createEscaper(unEscapeMap);
	df.templateSetting = {
		evaluate: /<%([\s\S]+?)%>/g,
		interpolate: /<%=([\s\S]+?)%>/g,
		escape: /<%-([\s\S]+?)%>/g,
	...
	df.template = function (text, settings, oldSettings) {
		if(!settings && oldSettings) settings = oldSettings;

		settings = df.defaults({}, settings, df.templateSetting);
	...
	df.mixin = function(obj) {

		return df.each(df.functions(obj), function(name){
			var func = df[name] = obj[name];
			df.prototype[name] = function(){
				var args = [this._wrapper];
				push.apply(args, arguments);
				return chainResult(this, func.apply(df, args));
			}
		})
	};
	df.chain = function (obj) {
		var instance = df(obj);
